...
%%
%% Simulation of CMi fiber with 11 fold increased expression of Nav1.8
%% This script generates Fig 11B
...
gNa_max_1_8 = f_1_8*gNa_max*11; % increase Nav1.8 by factor 11 compared to control
...
% load resting state of CMi fiber with increased Nav1.8 as initial condition
load("IC_CMi_1_8.mat","IC")
...
%% simulation
i0 = 40; % injected current per area [uA/cm²] 
[T_AP,Y_AP] = ode23s(@HH_model,[T_start,T_end],IC);

i0 = 30; % injected current per area [uA/cm²] 
[T_AP_30,Y_AP_30] = ode23s(@HH_model,[T_start,T_end],IC);

%% plotting
figure()
hold on
plot(T_AP_30,Y_AP_30(:,1),'-','LineWidth',2,'Color','#B7B7B7') % current injection 30 uA/cm²
plot(T_AP,Y_AP(:,1),'k-','LineWidth',2) % current injection 40 uA/cm²
xticks([0,25,50])
...
ylabel ('U [mV]')
set(gca,'FontSize',20)
legend('30 \muA/cm^2', '40 \muA/cm^2')
grid on
pbaspect([1 1 1])
exportgraphics(gca,'Fig11B.png','Resolution',300) 
